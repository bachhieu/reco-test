...
// @Accept       json
// @Produce      json
// @Param        request       body      user_do.CreateUserReq  true  "Create User request"
...
// @Router       /admin/users/sign-up [post]
func (api *UsersAPI) SignUp(ctx *fiber.Ctx) error {
	input := &user_do.CreateUserReq{}

	if err := ctx.BodyParser(input); err != nil {
		err := v_proto.NewRpcError(int32(v_proto.VolioRpcErrorCodes_BAD_REQUEST), err.Error())
		v_log.V(1).WithError(err).Errorf("UsersAPI::Create::BodyParser - Error: %+v", err)
		return v_api.WriteError(ctx, err)
	}

	input.Role = "user"                              // default
	input.Provider = constant.PROVIDER_USER_INTERNAL // default
	res, err := api.usersController.Create(input)
	if err != nil {
